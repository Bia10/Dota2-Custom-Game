...
modifier_item_mjz_bogduggs_cudgel = class({})

--------------------------------------------------------------------------------

function modifier_item_mjz_bogduggs_cudgel:IsHidden() 
	return true
end

--------------------------------------------------------------------------------

function modifier_item_mjz_bogduggs_cudgel:IsPurgable()
	return false
end

--------------------------------------------------------------------------------

function modifier_item_mjz_bogduggs_cudgel:GetPriority()
	return MODIFIER_PRIORITY_SUPER_ULTRA
end

--------------------------------------------------------------------------------

function modifier_item_mjz_bogduggs_cudgel:OnCreated( kv )
	if IsServer() then
		if self:GetAbility().iLevel ~= nil then
			self:GetAbility():SetItemLevel(self:GetAbility().iLevel)
		end
	end
	...
	self.item_level = self:GetAbility():GetLevel()
	self.bonus_strength_level_0 = self:GetAbility():GetSpecialValueFor( "bonus_strength_level_0" )
	self.bonus_strength_per_level = self:GetAbility():GetSpecialValueFor( "bonus_strength_per_level" )
	self.bonus_damage_level_0 = self:GetAbility():GetSpecialValueFor( "bonus_damage_level_0" )
	self.bonus_damage_per_level = self:GetAbility():GetSpecialValueFor( "bonus_damage_per_level" )
	self.bonus_strength = self.bonus_strength_level_0 + self.bonus_strength_per_level * self.item_level
	self.bonus_damage = self.bonus_damage_level_0 + self.bonus_damage_per_level * self.item_level
end

--------------------------------------------------------------------------------

function modifier_item_mjz_bogduggs_cudgel:DeclareFunctions()
	local funcs = 
	{
...
function modifier_item_mjz_bogduggs_cudgel:GetModifierBonusStats_Strength( params )
	return self.bonus_strength
end

--------------------------------------------------------------------------------

function modifier_item_mjz_bogduggs_cudgel:GetModifierPreAttack_BonusDamage( params )
	return self.bonus_damage
end

--------------------------------------------------------------------------------

function modifier_item_mjz_bogduggs_cudgel:GetModifierAttackSpeedBonus_Constant( params )
	if self.flAttackSpeedReduction == nil then
		return 0
	end
...
function modifier_item_mjz_bogduggs_cudgel:OnAttackLanded( params )
	if IsServer() then
		...
		if Attacker ~= nil and Attacker == self:GetParent() and Attacker:IsRangedAttacker() == false and Target ~= nil then
			...
			EmitSoundOn( "sounds/weapons/hero/leshrac/split_earth.vsnd", Target )
