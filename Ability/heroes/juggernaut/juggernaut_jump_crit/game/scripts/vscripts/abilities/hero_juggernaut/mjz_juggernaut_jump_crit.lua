...
function OnSpellStart( keys )
    ...
    caster:EmitSound('juggernaut_jug_ability_omnislash_16')
...
function DoDamage(caster, target, ability)
    ...
    local crit_damage = ability:GetLevelSpecialValueFor("crit_damage", (ability:GetLevel() - 1))

    local dmg_to_target = caster:GetAverageTrueAttackDamage(caster) * crit_damage / 100
    ...
    caster:PerformAttack (
        ...
        true,       -- bool bUseProjectile
        false,      -- bool bFakeAttack
        true        -- bool bNeverMiss  可敌先机
    )
end

function DoDamage_old(caster, target, ability)
