...
function modifier_class:OnIntervalThink()
	...
	if unit then
		if self._PrimaryStatValue ~= unit:GetPrimaryStatValue() then
			self:_Init()
			self._PrimaryStatValue = unit:GetPrimaryStatValue()
		end
	end
end
...
function modifier_class:_Init( )
	...
	local bonus = unit:GetPrimaryStatValue() * (self.primary_attribute_per / 100)
